...
const seedAdmin = async () => {
  try {
    const usersToSeed = [
      ...
      {
        name: 'Demo Agent 1 (Typo Fix)',
        email: 'agent1_typo@agenticstore.com',
        phone: '9877988876',
        password: 'Agent@123',
        role: 'Agent',
        status: 'Active',
...
connectDB().then((isConnected) => {
  if (isConnected) {
    ...
    console.warn('Skipping auto-seeding because database is not connected.');
  }
});
...
mainRouter.use((req, res, next) => {
  ...
  if (mongoose.connection.readyState !== 1 && req.path !== '/test') {
    return res.status(503).json({ message: 'Database connection is unavailable. Please check if MongoDB is running locally or check your MONGO_URI.' });
  }
  next();
});
